...
export function listNextPlay(posW, posB, player) {
	...
	resTree = onlyBestMoves(resTree[0], resTree[1]) ;
	return resTree ;
}
...
function pionNextPlays (posPion, posW, posB, player) {
	...
	if(nexts[0] && attack <= (lgAtt = longueurAttack((nexts = findBestAttacks(nexts))))) {
		attack = lgAtt;
console.log('On retourne une ATTAK');
		return [posPion, nexts] ;
	}
	// Sinon, si on avait déjà une prise, pas la peine de continuer
	else if(attack) return null ;
	nexts = [] ;
 
	// Si le joueur est blanc, on descend [x+-1,y-1], sinon on monte (x+-1,y+1)
	var avance = (player == WHITE ? -1 : 1) ;
//	var tmpNexts = [[posPion[0]+1, posPion[1]+avance], [posPion[0]-1, posPion[1]+avance]] ;
...
// Renvoie pour plusieurs chemins d'attaque possible, le meilleur
function findBestAttacks(attack){
	...
	return [[tmp[0]], findBestAttacks(tmp[1])] ;
}
...
function lanceAttack(posPion, posW, posB, player, pionsMangés=[])
{
	...
	var tmpAttack = [[posPion[0]+1, posPion[1]+1]] ;	// Coin bas droit
	tmpAttack.push([posPion[0]-1, posPion[1]+1]) ; 		// Coin haut droit
	tmpAttack.push([posPion[0]+1, posPion[1]-1]);		// Bas gauche
	tmpAttack.push([posPion[0]-1, posPion[1]-1]);		// haut gauche
...
function dameNextPlays(posDame, posW, posB, player){
	var myPos = (player == WHITE ? posW : posB) ;
	var opPos = (player == WHITE ? posB : posW) ;
		...
		for(let y of [-1, 1]) {
			...
			while(isInBoard((tmpPos = [posDame[0]+x, posDame[1]+y])))
			{
				...
				if(isTaken(tmpPos, opPos) && 
					isInBoard((afterJump = [tmpPos[0]+plusMoinsX,tmpPos[1]+plusMoinsY]))
